                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-3 rounded-lg bg-black/40 border border-white/5">
                      <Label className="text-sm font-semibold text-zinc-200">Entrance Mode</Label>
                      <Switch 
                        checked={profile.themeConfig?.entranceMode?.enabled ?? false}
                        onCheckedChange={(checked) => updateProfile({
                          themeConfig: {
                            ...profile.themeConfig,
                            entranceMode: {
                              ...profile.themeConfig?.entranceMode,
                              enabled: checked
                            }
                          }
                        })}
                        data-testid="switch-entrance-mode"
                        className="data-[state=checked]:bg-purple-600"
                      />
                    </div>

                    {profile.themeConfig?.entranceMode?.enabled && (
                      <div className="space-y-3">
                        <Label className="text-sm font-semibold text-zinc-200 block">Entrance Type</Label>
                        <div className="grid grid-cols-2 gap-2">
                          {["particles", "glitch", "fade"].map((type) => (
                            <Button
                              key={type}
                              variant="ghost"
                              size="sm"
                              className={`capitalize text-xs h-8 rounded-lg font-semibold ${profile.themeConfig?.entranceMode?.type === type ? "bg-purple-600 text-white" : "text-zinc-400 hover:text-white hover:bg-white/5"}`}
                              onClick={() => updateProfile({
                                themeConfig: {
                                  ...profile.themeConfig,
                                  entranceMode: {
                              ...
                              data-testid={`entrance-type-${type}`}
                            >
                              {type}
                            </Button>
                          ))}
                        ...
                        <div className="space-y-2">
                          <Label className="text-xs font-semibold text-zinc-200 block">Custom Text</Label>
                          <input
                            type="text"
                            maxLength={20}
                            value={profile.themeConfig?.entranceMode?.text || "reveal"}
                            onChange={(e) => updateProfile({
                              themeConfig: {
                                ...profile.themeConfig,
                                entranceMode: {
                            ...
                            className="w-full px-3 py-2 text-xs rounded-lg bg-zinc-900 border border-white/10 text-white font-mono placeholder-zinc-600"
                            placeholder="reveal"
                            data-testid="input-entrance-text"
                          />
                        </div>
                      </div>
                    )}
